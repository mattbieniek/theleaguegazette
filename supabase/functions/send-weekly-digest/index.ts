...
Deno.serve(async (request: Request) => {
  ...
  let runId = "";
  try {
    const { data: weekly, error: weeklyError } = await supabase.from("team_weekly_results").select("season_year,week").order("season_year",{ascending:false}).order("week",{ascending:false}).limit(1).maybeSingle();
    ...
    const editionKey = isTest ? null : `${season}:${week}`;
