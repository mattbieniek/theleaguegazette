  ...
  select jsonb_build_object(
    ...
    'health', jsonb_build_object(
      'last_success_at', (
        ...
        where sr.sync_type = 'sleeper_automation'
          and sr.status = 'success'
          and (sr.sleeper_league_id is null or sr.sleeper_league_id = v_active_league_id)
      ),
      'last_failure_at', (
        select max(sr.completed_at)
        from public.sync_runs sr
      ...
      'stale_running_count', (
        ...
        where sr.sync_type = 'sleeper_automation'
          and sr.status = 'running'
          and sr.started_at < now() - interval '20 minutes'
          and (sr.sleeper_league_id is null or sr.sleeper_league_id = v_active_league_id)
      )
    ),
      ...
      from (
        select id, sync_type, status, started_at, completed_at,
          ...
          case when completed_at is null then null
            else round(extract(epoch from (completed_at - started_at)))::integer
          end as duration_seconds,
          details
        from public.sync_runs
        where (sleeper_league_id is null or sleeper_league_id = v_active_league_id)
          and (sync_type <> 'sleeper_automation' or sleeper_league_id is null or sleeper_league_id = v_active_league_id)
